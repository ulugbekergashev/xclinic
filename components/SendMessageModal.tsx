...
export const SendMessageModal: React.FC<Props> = ({
    ...
    const send = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast.success(t('sendmessagemodal.xabar_yuborildi'));
            onClose();
        } catch (err: any) {
            ...
            if (msg.includes('Bot not configured')) {
                ...
                toast.error(msg || t('sendmessagemodal.xabar_yuborilmadi'));
            }
        } finally {
            setSending(false);
        }
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={t('patients.details.sendMessage')} className="max-w-lg">
            <form onSubmit={send} className="space-y-4">
                ...
                <Select
                    ...
                    options={[
                        { value: '', label: templates.length ? t('sendmessagemodal.erkin_matn') : t('sendmessagemodal.shablon_yoq') },
                ...
                <div>
                    ...
                    <textarea
                        ...
                        rows={4}
                        required
                        placeholder={t('sendmessagemodal.xabar_matni_bemor_ismi')}
                ...
                {text.trim() && (
                    <div className="rounded-lg bg-canvas/40 border border-line p-3">
                        <p className="text-[11px] font-bold text-faint uppercase tracking-wider mb-1">
                            {t('sendmessagemodal.bemor_shuni_oladi')}
                        </p>
                ...
                <div className="flex justify-end gap-2 pt-2">
                    <Button type="button" variant="secondary" onClick={onClose} disabled={sending}>{t('ui.bekor')}</Button>
